...
type User struct {
	...
	AgreeUserAgreement *bool   `json:"agree_user_agreement,omitempty" gorm:"-"`
	AgreePrivacyPolicy *bool   `json:"agree_privacy_policy,omitempty" gorm:"-"`
	AccessToken        *string `json:"access_token" gorm:"type:char(32);column:access_token;uniqueIndex"` // this token is for system management
	Quota              int     `json:"quota" gorm:"type:int;default:0"`
	// BalanceUSD is derived from quota at runtime and is never persisted.
	BalanceUSD      float64        `json:"balance_usd" gorm:"-"`
...
func (user *User) Insert(inviterId int) error {
	...
	user.Group = normalizeNewUserGroup(user.Group)
...
func syncUserQuotaCacheFromDB(id int) {
	if !common.RedisEnabled {
		return
	}
	var quota int
	if err := DB.Model(&User{}).Where("id = ?", id).Select("quota").Find(&quota).Error; err != nil {
		common.SysLog("failed to read user quota after update: " + err.Error())
		return
	}
	if err := updateUserQuotaCache(id, quota); err != nil {
		common.SysLog("failed to synchronize user quota cache: " + err.Error())
	}
}
