...
export function ModelMutateDrawer({
  ...
  const billingGroups = useMemo(() => {
    ...
    const groups = safeJsonParse<Record<string, number>>(modelSettings.GroupRatio, { fallback: {}, silent: true })
      ...
      try {
        ...
        if (response.success) {
          ...
          if (modelSettings) {
            ...
            const billingExprByGroupMap = safeJsonParse<Record<string, Record<string, string>>>(
              modelSettings['billing_setting.billing_expr_by_group'], { fallback: {}, silent: true }
            )
            ...
            if (hasRatioConfig) {
              if (pricingMode === 'tiered' && values.billingExpr?.trim()) {
                ...
                if (group) {
                  billingModeByGroupMap[finalModelName] = { ...(billingModeByGroupMap[finalModelName] || {}), [group]: 'tiered_expr' }
                  billingExprByGroupMap[finalModelName] = { ...(billingExprByGroupMap[finalModelName] || {}), [group]: values.billingExpr.trim() }
                } else {
            ...
            const newBillingMode = normalizeJsonString(JSON.stringify(billingModeMap))
            if (newBillingMode !== normalizeJsonString(modelSettings['billing_setting.billing_mode'])) {
              updates.push({ key: 'billing_setting.billing_mode', value: newBillingMode })
            }
            const newBillingExpr = normalizeJsonString(JSON.stringify(billingExprMap))
            if (newBillingExpr !== normalizeJsonString(modelSettings['billing_setting.billing_expr'])) {
              updates.push({ key: 'billing_setting.billing_expr', value: newBillingExpr })
            }
        ...
        <Form {...form}>
          ...
          >
            ...
            <div className='space-y-4'>
              ...
              {pricingMode === 'tiered' ? (
                <div className='space-y-4'>
                  <FormField control={form.control} name='billingGroup' render={({ field }) => (
                    <FormItem>
                      <FormLabel>{t('Pricing group')}</FormLabel>
                      <Select items={billingGroups.map((group) => ({ value: group, label: group }))} value={field.value || billingGroups[0] || ''} onValueChange={(value) => field.onChange(value || '')}>
                        <FormControl><SelectTrigger><SelectValue placeholder={t('Select group')} /></SelectTrigger></FormControl>
                        <SelectContent alignItemWithTrigger={false}><SelectGroup>{billingGroups.map((group) => <SelectItem key={group} value={group}>{group}</SelectItem>)}</SelectGroup></SelectContent>
                      </Select>
