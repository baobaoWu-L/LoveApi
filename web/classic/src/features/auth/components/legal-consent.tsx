      ...
      <div className={cn('border-border/60 bg-muted/40 space-y-3 rounded-md border p-3', className)}>
        {hasUserAgreement && (
          <div className='flex items-start gap-3'>
            <Checkbox id='legal-user-agreement' checked={agreementChecked} onCheckedChange={(value) => onAgreementChange(value === true)} className='mt-0.5' />
            <Label htmlFor='legal-user-agreement' className='text-muted-foreground text-left text-xs leading-5 font-normal'>
              {t('I have read and agree to the')}{' '}
              <a href='/user-agreement' target='_blank' rel='noopener noreferrer' className='text-foreground/70 hover:text-foreground underline'>{t('User Agreement')}</a>
            </Label>
          </div>
        )}
        {hasPrivacyPolicy && (
          <div className='flex items-start gap-3'>
            <Checkbox id='legal-privacy-policy' checked={privacyChecked} onCheckedChange={(value) => onPrivacyChange(value === true)} className='mt-0.5' />
            <Label htmlFor='legal-privacy-policy' className='text-muted-foreground text-left text-xs leading-5 font-normal'>
              {t('I have read and agree to the')}{' '}
              <a href='/privacy-policy' target='_blank' rel='noopener noreferrer' className='text-foreground/70 hover:text-foreground underline'>{t('Privacy Policy')}</a>
            </Label>
          </div>
        )}
      </div>
